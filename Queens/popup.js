document.addEventListener('DOMContentLoaded', function() {
    const solveButton = document.getElementById('solveQueensBtn');
    ...
    if (solveButton) {
        solveButton.addEventListener('click', () => {
            ...
            chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                chrome.tabs.sendMessage(tabs[0].id, { action: "solveQueens" }, (response) => {
        ...
        console.error('Le bouton Solve Queens est introuvable.');
    }
});
...
function displayMatrix(matrix) {
    ...
    matrix.forEach(row => {
        ...
        row.forEach(cell => {
            ...
            td.innerText = cell ? 'Q' : '';  // Afficher 'Q' pour une reine
            tr.appendChild(td);
        });
